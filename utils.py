...
def pad_sents_char(sents, char_pad_token):
    ...
    def copy_sents(sents):
        new_sents = []
        for i in sents: # sentence loop
            s = []
            for j in i: # word loop
                w = []
                for k in j: # char loop
                    w.append(k)
                s.append(w)
            new_sents.append(s)
        return new_sents
    new_sents = copy_sents(sents)
    max_sents_len = len(new_sents[0])
    lens = []
    for s in new_sents:
        lens.append(len(s))
        if len(s) > max_sents_len:
            max_sents_len = len(s)
    sents_padded = []
    for s in new_sents:
        if len(s) < max_sents_len:
            for i in range(len(s), max_sents_len):
        ...
        for w in s:
            if len(w) > max_word_length:
                ...
                for j in range(len(w), max_word_length):
