...
class VocabEntry(object):
    ...
    def words2charindices(self, sents):
        ...
        # print ("show source", sents)
        sents_ids = []
        for sent in sents:
            word_indexes = []
            for word in sent:
                char_indexes = [self.start_of_word]
                for char in word:
                    if char in self.char2id:
                        char_indexes.append(self.char2id[char])
                    else:
                        char_indexes.append(self.char_unk)
                char_indexes.append(self.end_of_word)
                word_indexes.append(char_indexes)
            sents_ids.append(word_indexes)
        # print ("show op======", sents_ids)
        return sents_ids
    ...
    def to_input_tensor_char(self, sents: List[List[str]], device: torch.device) -> torch.Tensor:
        ...
        chars_var = torch.tensor(padded_sents_char, dtype=torch.long, device=device)
        result = chars_var.permute(1, 0, 2)
